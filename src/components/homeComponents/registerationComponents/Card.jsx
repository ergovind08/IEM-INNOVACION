...
const [textleft,setTextleft]=useState(0);
 ...
 if(index===1 || index===3){
  ...
  const imageAnimate = {
    ...
    onscreen: { x: 0, rotate:360,opacity:1,
          transition: {type: "spring", bounce: 0.4, duration: 3 }
    }  
  };
  const textAnimate = {
    offscreen: {x: -500,opacity:0,scale:0.8},
    onscreen: { x: 0, opacity:1,scale:1,
    transition: {type: "spring", bounce: 0.4, duration: 3 }
    }  
  };
  ...
  return (
    <>
      {(index === 0 || index === 1 || index === 3 || index === 2) && (
        <motion.div className={`w-full flex overflow-hidden relative px-6  py-12 items-center justify-center ${index === 1 || index === 3 ? 'md:flex-row-reverse' : 'md:flex-row'} flex-col flex-wrap md:gap-6 md:max-w-screen-xl  items-center`}
              initial={"offscreen"} whileInView={"onscreen"} viewport={{once:false}} transition={{staggerChildren:0.5}}>
          <motion.div className="flex flex-[0.75] md:flex-1 justify-center items-center overflow-hidden h-auto mx-4 md:w-auto"   variants={imageAnimate}
             >
            <img
              src={robot}
              alt="Robot"
          ...
          <div className="flex flex-[0.75] items-center flex-col md:gap-4 px-6 w-full md:w-auto h-auto" >
            <motion.h2 className="mt-[8px] font-bold text-center md:text-[60px] text-[40px] text-white undefined" variants={textAnimate}>
              {item?.heading}
            </motion.h2>
            <motion.div className="flex flex-col items-center w-full gap-2 md:gap-2 my-4 font-normal text-[18px] text-[#B0B0B0]" variants={textAnimate}>
              <ul>
                <li>{item?.p1}</li>
                <li>{item?.p2}</li>
                <li>{item?.p3}</li>
      ...
      {index === 4 && (
        <motion.div className="w-full flex overflow-hidden relative px-6 py-12 md:flex-row flex-col flex-wrap gap-8 md:gap-2 items-center"
        initial={"offscreen"} whileInView={"onscreen"} viewport={{once:false}} transition={{staggerChildren:0.5}}>
          <motion.div className="flex flex-[0.75] md:flex-1 justify-center items-center border-white overflow-hidden h-[460px] md:w-auto"
          ...
          <div className="flex-1 flex justify-center md:items-center flex-col">
            <motion.h2 className="mt-[8px] font-bold md:text-[64px] text-[40px] text-white undefined" variants={textAnimate}>
              Gaming Registration
            </motion.h2>
            <div className="mt-[48px] flex flex-wrap justify-between gap-[10px]">
              <motion.div className="flex-1 flex flex-col sm:max-w-[250px] min-w-[210px] md:items-center" variants={textAnimate}>
